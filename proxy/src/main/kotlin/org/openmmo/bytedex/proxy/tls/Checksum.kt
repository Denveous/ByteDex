...
object ChecksumFactory {
    fun create(size: Int, key: ByteArray): Checksum = when (size) {
        0 -> NoOpChecksum
        2 -> Crc16Checksum()
        in 4..32 -> HmacSha256Checksum(size, key)
        else -> error("unsupported checksum size: $size")
    }
}
...
class Crc16Checksum : Checksum {
    ...
    override fun verify(data: ByteBuf, expected: ByteArray): Boolean {
        ...
        val calc = hash(data)
        val want = ((expected[1].toInt() and 0xff) shl 8) or (expected[0].toInt() and 0xff)
        return calc == want
    }

    private fun hash(data: ByteBuf): Int {
        var sum = 0
        for (i in data.readerIndex() until data.writerIndex()) {
            val b = data.getByte(i)
            sum = (sum ushr 8) xor TABLE[(sum xor (b.toInt() and 0xff)) and 0xff]
        }
...
class HmacSha256Checksum(override val size: Int, key: ByteArray) : Checksum {
    init { require(size in 4..32) }
    ...
    override fun calculate(data: ByteBuf): ByteArray {
        val bytes = ByteArray(data.readableBytes()).also { data.getBytes(data.readerIndex(), it) }
        mac.update(bytes)
        mac.update(round(hashRound++))
    ...
    override fun verify(data: ByteBuf, expected: ByteArray): Boolean {
        ...
        val bytes = ByteArray(data.readableBytes()).also { data.getBytes(data.readerIndex(), it) }
        mac.update(bytes)
        mac.update(round(verifyRound++))
        return MessageDigest.isEqual(mac.doFinal().copyOfRange(0, size), expected)
    }

    private fun round(value: Int) = byteArrayOf(
